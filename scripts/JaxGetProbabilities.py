...
def main():
	...
	#			SETUP
	################################################################
	#50kb into 6-mers and then round up so it is divisible by 4
	seqLen = 50000
	max_length = seqLen // 6
	#round up
	while (max_length % 4 != 0):
		max_length += 1
	max_length = 8336 + 1

	print("Writing header")
	
	outfile = open("/run/SegmentNToutput_Jax.csv", "w")
	...
	#			Read in data
	#################################################################
	infileName = "/run/50_kb_chunks_accounting_for_Ns.fa"
	#infileName = "/run/reducedChunks.fa"
...
def RunModel(parameters, segments, seqLen):
	max_num_nucleotides = 8336
